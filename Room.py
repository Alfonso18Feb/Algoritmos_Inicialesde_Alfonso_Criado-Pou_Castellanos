...
class Room(HotelRoomType):
    ...
    def is_occupied(self,room_state):
        if room_state=='occupied':
            return True
        elif room_state=='free':
            return False
        else:
            TypeError('write if occupied or free')
    def check_in(self,room_number,room_type,room_price):
        if room_number=='free':
            return 'you chose room Nº{} that is a {} with a price of {} '.format(self.room_number,self.room_type,self.room_price) and self.room_state=='occupied'  
        else:
            return 'Room is already occupied'
    def check_out(self, room_state):
        if room_state=='occupied':
            return self.room_state=='free'
        else:
            NameError('there wasn`t any one in this room')
